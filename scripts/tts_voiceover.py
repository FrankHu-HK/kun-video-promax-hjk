...
===================================================================================
把"口播话术/脚本文本"变成真人配音audio。

设计原则（诚实版）：
  - 默认后端 pyttsx3：调用系统自带语音（Windows=SAPI5，含中文嗓音），
    **完全离线、零云端、零国外、零付费**，绝对诚实，永不破"两条底线"。
  - 可选后端 edge_tts：音质更natural，但走微软国外免费云（无 Key、免费），
    属于"可选升级"，enable时明确告知user这是国外云服务（不默认、不隐瞒）。
  - 两个后端都realavailable，不返回fakeaudio。

用法：
  # 默认本地离线配音（recommended，零云端）
...
# 常见中文嗓音（仅作hint/tip，实际以本机/edge_tts available列表为准）
DEFAULT_VOICES = {
    "pyttsx3": "",  # 留空=系统默认中文嗓音
    "edge_tts": "zh-CN-XiaoxiaoNeural",  # 微软小晓（女声，natural）
}

EDGE_CN_VOICES = [
    "zh-CN-XiaoxiaoNeural",   # 女声（recommended，natural）
...
def _read_text(text, text_file):
    if text_file:
        if not os.path.exists(text_file):
            raise SystemExit("E100: 话术filedoes not exist: %s" % text_file)
...
def generate_pyttsx3(text, out_path, voice="", rate=None):
    ...
    try:
        ...
        raise SystemExit(
            "E205: 未install pyttsx3（本地离线配音引擎）。\n"
            "请先run: pip install pyttsx3\n"
            "（Windows 自带 SAPI 中文嗓音，装完即可离线配音；"
            "或用 --backend edge_tts 走免费云，但那是国外服务）"
        )
    # pyttsx3 只稳妥支持 wav；若user要 mp3 则提醒
    if out_path.lower().endswith(".mp3"):
        logging.warning("  pyttsx3 后端只stableoutput wav，已自动改为 .wav")
    ...
    if not os.path.exists(out_path) or os.path.getsize(out_path) < 1000:
        raise SystemExit("E201: pyttsx3 generateaudiofailed（可能无available中文嗓音或authorization问题）")
    return out_path


def generate_edge_tts(text, out_path, voice=DEFAULT_VOICES["edge_tts"], rate=None):
    """可选升级后端：edge_tts（微软国外免费云，无 Key）。音质更natural。"""
    logging.warning(
    ...
    try:
        ...
        raise SystemExit(
            "E205: 未install edge_tts（可选升级后端）。\n"
            "请先run: pip install edge_tts\n"
    ...
    try:
        ...
        raise SystemExit("E201: edge_tts generatefailed: %s（check网络/嗓音名）" % e)
    if not os.path.exists(out_path) or os.path.getsize(out_path) < 1000:
        raise SystemExit("E201: edge_tts 未产出有效audio")
    return out_path
...
def list_voices(backend):
    if backend == "edge_tts":
        try:
            ...
            raise SystemExit("E205: 未install edge_tts，无法列嗓音；先 pip install edge_tts")
        import asyncio
        voices = asyncio.run(edge_tts.list_voices())
        cn = [v for v in voices if v["Locale"].startswith("zh-CN")]
        print("available中文嗓音（edge_tts，微软国外免费云）：")
        for v in cn:
    ...
    try:
        ...
        raise SystemExit("E205: 未install pyttsx3；先 pip install pyttsx3")
    engine = pyttsx3.init()
    voices = engine.getProperty("voices")
    print("本机available嗓音（pyttsx3，离线系统语音）：")
    for v in voices:
...
def main():
    ...
    ap.add_argument("--text-file", help="话术文本file（多行）")
    ...
    ap.add_argument("--voice", default="", help="嗓音名（pyttsx3 用系统 id blurry匹配；edge_tts 用如 zh-CN-XiaoxiaoNeural）")
    ap.add_argument("--rate", default=None, help="语速：pyttsx3 用整数 wpm(默认200)；edge_tts 用 +5pct/-10pct 写法")
    ap.add_argument("--list-voices", action="store_true", help="列出available嗓音后exit")
    args = ap.parse_args()
    ...
    logging.info("DONE -> 配音已generate: %s (零云端零付费)", out)
