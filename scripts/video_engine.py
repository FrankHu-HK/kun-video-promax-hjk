# -*- coding: utf-8 -*-
"""
video_engine.py (v2.8.0) - Workflow B/C 视频生成引擎（默认本地、零云端、零付费）
===================================================================================
设计原则（诚实版）：
  - 默认 --engine local：100% 本地、零云端、零付费、零国外服务。
  - Workflow B（动作迁移·本地）=「动作预检 + 换脸式动作迁移」：
      先用 pose_extract.py 分析参考视频动作清晰度，再拿目标照对参考视频做换脸，
      得到"目标人物做出参考视频动作"的视频（**保留原动作/背景/音乐**）。真实可跑。
  - Workflow C（AI 口播·本地）= 话术→本地真人配音(tts_voiceover.py：默认 pyttsx3 离线
      零国外，可选 edge_tts 免费云)→与产品视频/主播图合成口播 mp4（可选背景音乐）。
      **真实可出片**，零云端零付费；"真人脸驱动数字人"需本地模型(如 Wav2Lip)，本技能
      不内置，明确标注为可选本地扩展，不伪造。
  - 导演模式（--mode director）：确定性触发"导演策划层"（AI 导演引擎/分镜/提示词），
      不再需要用户"猜怎么开口"。
  - 云端引擎（agnes/nvidia/seedance/kling）：**真实对接尚未实现**，选择它们会明确报错
    E200「请用 --engine local」，绝不返回假视频。这是为了守住"不谎报"的诚信底线。
...
# ============ 统一错误码 ============
ERROR_CODES = {
    "E200": "未实现真实云端对接，请改用 --engine local（零云端零付费）",
    "E201": "引擎调用失败 / 未实现",
    "E205": "本地等价方案依赖脚本缺失",
}
...
ENGINE_ROUTES = {
    "local": {
        "display_name": "本地等价方案（默认 · 真实可跑）",
        "type": "local",
        "is_free": True,
        "is_foreign": False,
        "is_implemented": True,
        "supports": ["B", "C"],
        "description": "零云端、零付费、零国外。B=换脸式动作迁移(保留原动作)；C=产品提取+话术。",
    },
    "agnes": {
        "display_name": "AGNES 视频生成（真实对接未实现）",
        "type": "cloud_free",
        "is_free": True,
        "is_foreign": False,
        "is_implemented": False,
        "supports": ["B", "C"],
        "description": "规划中：真实 API 对接尚未实现，选择会报错并引导用 local。",
    },
    "nvidia": {
        "display_name": "NVIDIA 视频生成（真实对接未实现）",
        "type": "cloud_free",
        "is_free": True,
        "is_foreign": False,
        "is_implemented": False,
    ...
    "seedance": {
        "display_name": "Seedance 2.0（真实对接未实现）",
        "type": "cloud_trial",
        "is_free": False,
        "is_foreign": False,
        "is_implemented": False,
    ...
    "kling": {
        "display_name": "Kling 3.0（真实对接未实现）",
        "type": "cloud_paid",
        "is_free": False,
        "is_foreign": False,
        "is_implemented": False,
...
def run_motion_qc(ref_path, out_json, pose_model=""):
    ...
    try:
        ...
        return None, "动作预检异常: %s" % e
    if r.returncode != 0:
        return None, "动作预检失败: %s" % r.stderr.strip()[:200]
    try:
        with open(out_json, "r", encoding="utf-8") as f:
            d = json.load(f)
        return d.get("motion_score"), d.get("verdict", "")
    except Exception:
        return None, "动作预检结果解析失败"
...
def compose_koubo(video_path, image_path, audio_path, bgm_path, out_path, print_cmd=False):
    """把配音合成到产品视频 / 主播图，产出口播 mp4（ffmpeg）。

    - 有产品视频：替换音轨为配音（可选叠加背景乐）。
    - 仅主播图：图片缓动(ken-burns)+配音+可选背景乐 → 口播幻灯片。
    返回 (ok, cmd_str)。print_cmd=True 仅打印命令不执行（用于无 ffmpeg 环境验证）。
    """
    ff = _ffmpeg_exe()
    if video_path and os.path.exists(video_path):
        ...
        return False, "既无产品视频也无主播图，无法合成视频（仅产出配音音频）"

    if print_cmd:
        logging.info("  [print-cmd] 将执行: %s", " ".join('"%s"' % c if " " in c else c for c in cmd))
        return True, " ".join(cmd)
    logging.info("  合成口播视频（%s）…", target)
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=1800)
    except Exception as e:
        logging.error("  ffmpeg 执行异常: %s", e)
        return False, str(e)
    if r.returncode != 0:
...
def run_local_fallback(workflow, ref_path, photo_path, output_path, pose_model="", no_motion_qc=False,
    ...
    B: 动作预检(pose_extract) → 换脸式动作迁移(faceswap_pro)，保留原动作/背景/音乐。
    C: 产品提取(product_extract) → 话术本地可用（真人口播下一轮）。
    """
    if workflow == "B":
        # 步骤 1：动作预检
        if not no_motion_qc:
            ...
            if score is not None:
                logging.info("  动作可迁移性评分=%s/100 | %s", score, verdict)
                if score < 35:
                    logging.warning("  ⚠️ 参考视频动作/清晰度偏弱，迁移效果可能有限；建议换动作更明显的素材。")
            else:
                logging.warning("  ⚠️ %s（跳过预检，直接换脸）", verdict)
        else:
            logging.info("  已跳过动作预检（--no-motion-qc）")

        # 步骤 2：换脸式动作迁移（保留原动作）
        fs = _script("faceswap_pro.py")
        if not os.path.exists(fs):
            raise SystemExit("E205: 缺少 faceswap_pro.py，无法执行动作迁移")
        logging.info("  执行换脸式动作迁移：把 %s 的脸贴到 %s（保留原动作/背景/音乐）",
        ...
        # 步骤 1：话术 → 本地真人配音（默认 pyttsx3 离线零国外；可选 edge_tts 免费云）
        if not script:
            raise SystemExit("Workflow C 必须提供 --script（口播话术）；可加 --product/--anchor 合成视频")
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        try:
            import tts_voiceover as tts
        except Exception as e:
            raise SystemExit("E205: 无法加载 tts_voiceover.py（口播配音引擎）: %s" % e)
        ...
        logging.info("  ✅ 真人配音已生成: %s", voice_out)

        # 步骤 2：选填——提取产品参考图（可选，便于你挑封面/素材）
        ...
        if os.path.exists(pe) and ref_path and os.path.exists(ref_path) and ref_path.endswith((".mp4", ".mov", ".avi", ".mkv")):
            ...
            try:
                ...
                logging.info("  已本地提取产品参考图到: %s", out_dir)
            except Exception as e:
                logging.warning("  ⚠️ 产品图提取跳过（不影响口播出片）: %s", e)

        # 步骤 3：合成口播视频（有产品视频→替换音轨；有主播图→口播幻灯片；都没有→仅配音音频）
        if output_path.endswith((".mp4", ".mov", ".avi", ".mkv")):
            ...
            if not ok:
                logging.warning("  ⚠️ 视频合成未完成（%s）；配音音频已产出: %s", cmd, voice_out)
                return True  # 配音已成功，视频合成为可选增强
            logging.info("  口播视频已生成: %s", output_path)
        else:
            logging.info("  输出非视频路径，已产出配音音频: %s", voice_out)
        return True
    return False


def require_local_or_error(engine_name):
    """云端引擎真实对接未实现 → 诚实报错，绝不返回假视频。"""
    route = ENGINE_ROUTES.get(engine_name)
    if route and route.get("is_implemented"):
        return
    raise SystemExit(
        "E200: 云端引擎 '%s' 真实对接尚未实现（本技能默认本地零云端）。\n"
        "请改用: --engine local （零云端、零付费、真实可跑）。\n"
        "虚伪地返回一个假视频违背本技能的诚实底线，已禁用。" % engine_name
    )
...
def run_director():
    """导演策划层【确定性触发入口】——不再需要用户"猜怎么开口"。

    说明：导演策划由 AI 助理按 SKILL.md 的导演层规则完成（纯本地推理、零依赖、零费用）。
    本函数给出标准化出片协议与开场提问，作为明确入口；也可直接对助理说
    "帮我策划一个关于XX的视频" 触发同样流程。
    """
    logging.info("=" * 60)
    logging.info("🎬 导演策划模式（Director Mode）已激活 · 本地免费 · 零云端")
    logging.info("=" * 60)
    logging.info("下一步请告诉我这 4 项，我会产出《视频导演方案》：")
    logging.info("  ① 视频主题/卖点（如：夏日清透防晒霜种草）")
    logging.info("  ② 发布平台（抖音/视频号/小红书/快手，影响节奏与画幅）")
    logging.info("  ③ 时长目标（如 15s / 30s / 60s）")
    logging.info("  ④ 风格偏好（如 快节奏卡点 / 温柔测评 / 反转剧情）")
    logging.info("-" * 60)
    logging.info("《视频导演方案》标准化输出（9 要素）：")
    for i, el in enumerate([
        ...
        "音乐/情绪建议",
        "视觉语言与配色",
        "本地生产层映射（换脸/动作迁移/口播/4K升频哪个做）",
        "一致性自查清单",
        "出片前目检要点（本技能不读图，画面由你确认）",
    ], 1):
...
def main():
    ap = argparse.ArgumentParser(
        description="视频生成引擎（本地优先，零云端零付费 · 诚实版）",
        formatter_class=argparse.RawTextHelpFormatter,
    )
    ap.add_argument("--mode", default="video", choices=["video", "director"],
                    help="video=生成视频(需 --workflow)；director=进入导演策划层(确定性触发)")
    ap.add_argument("--workflow", choices=["B", "C"], help="B=动作迁移, C=口播带货（--mode video 时必填）")
    ap.add_argument("--engine", default="local", choices=list(ENGINE_ROUTES.keys()),
                    help="引擎（默认 local=本地零云端；其余云端引擎尚未实现，选了会报错引导用 local）")
    ap.add_argument("--video", help="参考视频（Workflow B 用）")
    ap.add_argument("--photo", help="目标人脸照（Workflow B 用）")
    ap.add_argument("--product", help="产品视频（Workflow C 用，可选）")
    ap.add_argument("--anchor", help="主播参考照/产品图（Workflow C 用，可选）")
    ap.add_argument("--script", help="口播话术（Workflow C 用；无产品视频时也可只产配音）")
    ap.add_argument("--bgm", help="背景音乐（Workflow C 可选叠加）")
    ...
    ap.add_argument("--pose-model", default="", help="pose_landmarker_full.task（启用姿态预检增强）")
    ap.add_argument("--no-motion-qc", action="store_true", help="跳过动作预检（Workflow B）")
    ap.add_argument("--print-cmd", action="store_true", help="仅打印 ffmpeg 合成命令不执行（无 ffmpeg 环境验证用）")
    ap.add_argument("--out", default="video_engine_output.mp4", help="输出路径")
    ...
    logging.info("🎬 视频生成引擎 v2.9.0  workflow=%s engine=%s (%s)",
    ...
    if not ok:
        raise SystemExit("E201: 本地方案执行失败，请检查上方日志")

    if args.workflow == "B":
        extra = {"method": "换脸式动作迁移（保留原动作/背景/音乐）"}
        write_report(args.out, args.workflow, "local", extra)
        logging.info("DONE -> %s (本地动作迁移，零云端零付费)", args.out)
    else:
        write_report(args.out, args.workflow, "local",
                     extra={"method": "AI口播：本地真人配音+合成(零云端零付费)"})
